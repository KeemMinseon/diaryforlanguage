 ...
 * or a made-up, not-in-any-dictionary term (e.g. "종관 제한(오타로 추정)"
 * or "종관 제한(오용)" for a kanji run that probably wasn't a real word)
 * — visibly not a real gloss, and not something to display in a word
 * list meant to look like an actual dictionary.
 *
 ...
 */
const UNCERTAIN_MEANING_RE = /오타|추정|오용|불확실|확실(하지|치)\s*않|알\s*수\s*없/;
