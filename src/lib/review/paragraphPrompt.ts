...
- readings: 문단에 나오는 한자가 포함된 부분과 가타카나 단어를 **하나도 빠짐없이 전부** 처리하세요. 한자가 여러 군데 나오면 그 개수만큼 항목을 제출해야 합니다 (일부만 골라서 내면 안 됩니다). 같은 단어가 여러 번 나와도 한 번만 제출하면 됩니다.
  ...
  - "text"는 학습자가 문단에 실제로 쓴 표기와 한 글자도 다르지 않게 정확히 일치해야 합니다. 문단에 오타나 어색한 표현, 한국어가 섞여 있어도 절대 고치거나 정규화하지 말고 원문 그대로 복사하세요 — 정확히 일치하지 않으면 이 항목은 화면에 아예 표시되지 않습니다.`;
